...
(define (intersection LA LB)
   (cond ((null? LA) '())
        ...
        (else (make-set(intersection (cdr LA) LB)))))
...
(define (set-equal? SA SB)
  (define (delete elem L)
     (cond ((null? L) '())
           ((equal? elem (car L)) (cdr L))
           (else (cons (car L) (delete elem (cdr L))))))
  (if (equal? (make-set SA) (make-set SB)) #t
      (if (null? SA) #f
          (if (null? SB) #f
              (if (element? (car (make-set SA)) (make-set SB)) (set-equal? (cdr SA) (delete (car (make-set SA)) (make-set SB)))
                  (set-equal? (cdr SA) SB))))))
...
(define (set-difference LA LB)
  (cond ((null? LA) '())
        ((null? LB) (make-set LA))
        ((set-equal? LA LB) '())
        ((not(element? (car LA) LB)) (cons (car LA)(set-difference (cdr LA) LB)))
...
(define (sym-diff LA LB)
  (append (set-difference LA LB) (set-difference LB LA)))
...
(define (disjoint? SA SB)
  (if (equal? (intersection SA SB) '()) #t #f))
...
(define (remove e S)
  (cond ((null? S) '())
           ((equal? e (car S)) (cdr S))
           (else (cons (car S) (remove e (cdr S))))))
