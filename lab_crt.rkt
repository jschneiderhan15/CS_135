...
(define (euclid-gcd a b)
  (cond ((and (eq? a 0) (eq? b 0)) 0)
        ((and (eq? a 0) (not(eq? b 0)) b))
        ((and (not(eq? a 0)) (eq? b 0)) a)
...
(define (CRT-exists? cong-sys)
  (cond ((null? cong-sys) #t)
        ((not(CRT-help cong-sys (cadar cong-sys))) #f)
        (else (CRT-exists? (cdr cong-sys)))))

(define (CRT-help cong-sys num)
  (cond ((null? cong-sys) #t)
        ((eq? (cadar cong-sys) num) (CRT-help (cdr cong-sys) num))
        ((not(eq? 1 (euclid-gcd (cadar cong-sys) num))) #f)
        (else (CRT-help (cdr cong-sys) num))))
...
(define (m cong-sys)
  (apply * (m-helper cong-sys)))

(define (m-helper cong-sys)
  (cond ((null? cong-sys) '())
        (else (cons (cadar cong-sys) (m-helper(cdr cong-sys))))))
...
(define (CRT-helper cong-sys m)
  (apply + (CRT-helper-helper cong-sys m)))

(define (CRT-helper-helper cong-sys m)
  (if (null? cong-sys) '(0) (append (list (* (* (/ m (cadar cong-sys)) (caar cong-sys)) (mul-inv (/ m (cadar cong-sys))
                                                                    (cadar cong-sys)))) (CRT-helper-helper (cdr cong-sys) m))))
...
(define (CRT cong-sys)
  (if (not(CRT-exists? cong-sys)) -1 (modulo (CRT-helper cong-sys (m cong-sys)) (m cong-sys))))