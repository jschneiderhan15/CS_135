...
(define (indegree Es v)
  (cond ((null? Es) 0)
        ((equal? (cadar Es) v) (+ 1 (indegree (cdr Es) v)))
...
(define (find-top G)
  (find-top-helper (get-vertices G) G))

(define (find-top-helper verts G)
  (cond ((null? verts) (tsort-error))
        ((zero-indegree? (get-edges G) (car verts)) (car verts))
        (else (find-top-helper (cdr verts) G))))

(define (zero-indegree? Es v)
  (if (equal? (indegree Es v) 0) #t #f))
...
(define (pop-vertex G v)
  (append (list(pop-edge (car G) v)) (list(pop-vertexes (cadr G) v))))

(define (pop-vertexes G v)
  (cond ((null? G) '())
        ((or (equal? (caar G) v) (equal? (cadar G) v)) (pop-vertexes (cdr G) v))
        (else (append (list(car G)) (pop-vertexes (cdr G) v)))))
                            
(define (pop-edge edges v)
  (cond ((null? edges) '())
        ((equal? (car edges) v) (pop-edge (cdr edges) v))
        (else (append (list(car edges)) (pop-edge (cdr edges) v)))))
...
(define (tsort G)
  (cond ((null? (car G)) '())
        (else (append (list(find-top G)) (tsort (pop-vertex G (find-top G)))))))


(define (pos lst) 
	(cond ((null? lst) '())
	          ((> (car lst) 0) (append (list (car lst)) (pos (cdr lst))))
	          (else (pos (cdr lst)))))
