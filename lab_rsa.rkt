...
(define (generate p q e)
 (append(list(list(* p q) e) (list (* p q) (mul-inv e (lcm(- 1 p) (- 1 q)))))))
...
(define (mod-exp b e m)
  (cond ((eq? e 0) 1)
        ((eq? (modulo e 2) 0) (modulo (expt (mod-exp b (/ e 2) m) 2) m))
...
(define (encrypt plaintext key)
 (encryptify (string-> list plaintext) key)) 

(define (encryptify plainList key)
  (cond ((null? plainList) '())
        (else (append (list (mod-exp (char->integer(car plainList)) (cadr key) (car key))) (encryptify (cdr plainList) key)))))
...
(define (decrypt ciphertext key)
  (charify(decryptify ciphertext key)))

(define (charify L)
  (list->string L))

(define (decryptify ciphertext key)
  (cond ((null? ciphertext) '())
        (else (append (list (integer->char(mod-exp (car ciphertext) (cadr key) (car key)))) (decryptify (cdr ciphertext) key)))))
