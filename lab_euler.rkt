...
(define (get-vertices G)
  (usort(get-vertices-helper G)))

(define (get-vertices-helper G)
  (cond ((null? G) '())
        (else (append (append (list (caar G)) (list (cadar G))) (get-vertices-helper (cdr G))))))
...
(define (degree G v)
  (cond ((null? G) 0)
        ((and(eq? (caar G) v) (eq? (cadar G) v)) (+ 2 (degree (cdr G) v)))
        ((or(eq? (caar G) v) (eq? (cadar G) v)) (+ 1 (degree (cdr G) v)))
        (else (degree (cdr G) v))))
...
(define (all-even? G)
  (all-even-helper (degree-list G (get-vertices G))))

(define (all-even-helper G)
  (cond ((null? G) #t)
        ((not(eq? 0 (modulo (car G) 2))) #f)
        (else (all-even-helper (cdr G)))))

(define (degree-list G V)
  (cond ((null? V) '())
         (else (append (list(degree G (car V))) (degree-list G (cdr V))))))
...
(define (adjacent G v)
  (usort(adjacent-helper G v)))

(define (adjacent-helper G v)
  (cond ((null? G) '())
        ((eq? (caar G) v) (append (list(cadar G)) (adjacent-helper (cdr G) v)))
        ((eq? (cadar G) v) (append (list(caar G)) (adjacent-helper (cdr G) v)))
        (else (adjacent-helper (cdr G) v))))
...
(define (connected? G)
  (if (null? G) #t
      (if (eq? (car (usort(connected-helper G (get-vertices G)))) 1) #t #f)))

(define (connected-helper G V)
  (cond ((null? V) '())
         ((equal? (get-vertices G) (usort(dfs G '() (car V))) ) (append '(1) (connected-helper G (cdr V))))
         (else (append '(0) (connected-helper G (cdr V))))))
