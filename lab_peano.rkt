...
(define (pred p)
  (if (eq? p z) (eopl:error "pred error: zero has no predecessor!") (car p)))
...
;; Type signature: (p- peano peano) -> peano
(define (p- a b)
  (cond ((and (pz? a) (not(pz? b))) (eopl:error "<subtraction error: a < b!"))
        ((pz? a) b)
        ((pz? b) a)
...
(define (p-prime? p)
  (if (p> p (succ z)) (primeHelp p (succ (succ z))) #f))

(define (primeHelp a b)
  (cond ((p= a b) #t)
        ((pz? (p% a b)) #f)
        (else (primeHelp a (succ b)))))
